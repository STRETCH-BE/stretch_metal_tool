/**
 * Locale resolution + number/date formatting.
 * File path: /lib/i18n.ts
 *
 * Locale priority: the signed-in user's profile.locale → the `locale`
 * cookie (set by the language switcher on public pages) → Accept-Language
 * → "pl". UI strings never live in components: they come from the typed
 * dictionaries in /content (PL) and /content/en (EN) via getContent().
 *
 ...
 * 0.01 here, at the display boundary only.
 */
