 ...
 *
 * Invite: the service-role client's auth.admin.inviteUserByEmail with
 * {full_name, role, locale} in the user metadata — the DB trigger
 * handle_new_user copies them into the profile row. Missing service key →
 * "notConfigured" (no throw). Role change: the pure guard in
...
export async function inviteUserAction(_prev: InviteFormState, formData: FormData): Promise<InviteFormState> {
  ...
  try {
    ...
    const { data, error } = await admin.auth.admin.inviteUserByEmail(parsed.data.email, {
      data: { full_name: parsed.data.full_name || null, role: parsed.data.role, locale: parsed.data.locale },
    ...
    await logAudit({
      ...
      after: { email: parsed.data.email, role: parsed.data.role, locale: parsed.data.locale },
    });
  } catch (error) {
    console.error("[admin/users] invite failed", error);
