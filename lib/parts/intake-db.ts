 ...
 * (quotes/<quoteId>/…, see lib/files/storage.ts). Base-name matching is
 * done in TypeScript with the same helper the intake uses
...
export function createIntakeDb(client: IntakeClient): IntakeDb {
  return {
    ...
    async findPartByBaseName(quoteId, base): Promise<ExistingPart | null> {
      const { data, error } = await client
        .from("parts")
        .select("id, name, geometry, annotations, thickness_mm, material_code, created_at")
        .eq("quote_id", quoteId)
        .eq("source", "dxf")
    ...
    async findPdfByBaseName(quoteId, base): Promise<IntakeFile | null> {
      const { data, error } = await client
        .from("files")
        .select("id, original_name, storage_path, sha256, created_at")
        .eq("kind", "pdf")
        .like("storage_path", `quotes/${quoteId}/%`)
        .order("created_at", { ascending: false });
      if (error) throw new Error(`files by name: ${error.message}`);
      const row = (data ?? []).find((f) => baseNamesMatch(f.original_name, base));
