...
| Stage | What it does |
|---|---|
| `parse.ts` | `dxf-parser` wrapper. LINE, ARC (a 360° arc becomes a CIRCLE), CIRCLE, LWPOLYLINE/POLYLINE (bulges → exact arcs), SPLINE (rational NURBS — group 41 weights are read from the raw text by handle since dxf-parser drops them — or fit points, adaptive ≤ 0.05 mm), ELLIPSE (flattened), INSERT (exploded recursively with translate/scale/rotate). Header: `$ACADVER`, `$INSUNITS` (1 inch → ×25.4, 0/missing → unknown), `$EXTMIN/MAX`, layer names. Everything else (POINT, TEXT, DIMENSION, HATCH, …) is counted by type + layer from a raw scan of the ENTITIES section (dxf-parser skips unknown types silently). Throws `DxfFormatError` only when there is no `SECTION` group. |
...
| `loops.ts` | Chain by endpoints in three passes: peel leaves (an end nothing touches can never close — overshoots, tails, candidates), then a depth-first loop search that tries branches smallest-turn first and backtracks on dead ends (closes on any revisited chain vertex; a bridge prefix is released; global step budget `DFS_BUDGET_BASE + DFS_BUDGET_PER_UNIT × units`), then greedy open chains for the rest. Exact areas (shoelace + signed circular segments), exact arc lengths, true arc bbox, flattened points, circle detection (CIRCLE, or radius spread < 1 %). Near-closed chains (and single arcs healed shut) within tolerance are closed (`loopsClosed`). |
| `classify.ts` | Role overrides > layer role > geometry. Containment tree of closed loops (samples nudged towards the inner centroid so touching loops resolve); frames are ignored — a top-level true rectangle NOT on a named cut layer that holds a title block (child rectangle on two of its edges), or encloses ≥ 2 loops with children, or encloses a loop with children and shares no layer with its children; a rectangle on `IV_OUTER_PROFILE`/`CUT`, or on layer `0` around same-layer geometry, is always a part (a window with an island is not a frame). Each remaining top-level loop is a part group (`partIndex`, 0 = largest = `outerLoopId`); nested loops are holes; open chains inside a part are candidates (`unknown`) unless the layer says bend/weld/engrave or the user tagged them — an open line on a cut layer or layer `0` is still a candidate; outside → noise/ignore. |
| `measure.ts` | Cut length (outer + holes + open chains answered "cut"; every member of a closed contour counts, a weld/engrave tag on an edge adds an operation and never removes the cut), pierces (1 + holes + open cuts), `openCutsLengthMm`/`openCuts`, bbox, blank (+2 × margin), areas (islands add back), mass = area·t·ρ/1e9, holes with thread suggestions (`threads.ts`), bend lines and engrave length from the entities of THIS part group only (`loop.partIndex`; unplaced marking lines go to the part whose bbox holds them, or to the only part), smallest contour, slow contours (< 10 × t). |
...
| `amber_forming_unknown` | no bend lines (layer or tagged; weld/engrave/cut-tagged open lines do not count), and the name or PDF text contains a forming stem (`FORMING_HINT_STEMS`: bend/bent/fold/kant/roll/ohyb/biegen/walc/zwijan/gięc/gięt/giąć — "plech" is not one) — until forming/roll/bends are given |
...
prompt, synthetic DXFs built with `test/geometry/dxf-builder.ts`, review
regressions (`regressions.test.ts`: frames, overshoots, per-part measures,
forming triage, tagged edges, open cuts, 360° arcs, rational splines), export
