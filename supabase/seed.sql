...
--   1. If no quote references that version, the version row is deleted first
--      (cascading to every rate_* row) and re-inserted, so an edited seed
--      lands on re-run. rate_versions.on delete restrict from quotes plus the
--      *_immutable triggers would refuse the delete otherwise, so the DO block
--      checks first and leaves a referenced version untouched (NOTICE).
--   2. Every insert is `on conflict do nothing`, so a re-run on a referenced
--      version — or on a database where the admin already created and
--      activated a newer version — neither errors nor duplicates.
...
--   4. Machines are `on conflict (code) do nothing`: admin edits made in the
--      machines editor survive a re-run. Change limits in the UI, not here.
--
...
-- ─── 0. Reset the seeded version when nothing depends on it ────────────────
do $$
declare
...
begin
  if exists (select 1 from public.quotes where rate_version_id = v_version) then
    raise notice 'seed.sql: rate version % is referenced by quotes — existing rate rows left untouched', v_version;
  else
...
-- ─── 1. Rate version ───────────────────────────────────────────────────────
-- Label keeps "[CONFIRM]" on purpose: it is visible in the admin rate editor
-- until the admin clones the version and gives the copy a real label.
...
-- price_per_kg bands: ascending, each applies to thickness ≤ maxThicknessMm;
-- the last band uses 999 as an open-ended top. Density kg/m³, Rm N/mm² for
-- the press-brake force check (spec 8.3). Sheet formats drive the "blank
-- fits a sheet" check; scrap % is the default the user can override per item.
insert into public.materials (
  rate_version_id, code, name, family, density_kg_m3, rm_n_mm2, price_per_kg, sheet_formats, scrap_pct_default, placeholder
) values
  ('00000000-0000-4000-8000-000000000001', 'S235', 'S235JR structural steel', 'mild_steel', 7850, 400,
  ...
  ('00000000-0000-4000-8000-000000000001', 'S355', 'S355J2 structural steel', 'mild_steel', 7850, 510,
    ...
    '[{"lengthMm":3000,"widthMm":1500},{"lengthMm":2500,"widthMm":1250},{"lengthMm":2000,"widthMm":1000}]'::jsonb,
    25.00, true),  -- [CONFIRM] scrap %
  ('00000000-0000-4000-8000-000000000001', 'DC01', 'DC01 cold-rolled sheet', 'mild_steel', 7850, 350,
    '[{"maxThicknessMm":999,"pricePerKg":1.05}]'::jsonb,  -- [CONFIRM] €/kg flat
    '[{"lengthMm":3000,"widthMm":1500},{"lengthMm":2500,"widthMm":1250},{"lengthMm":2000,"widthMm":1000}]'::jsonb,
    25.00, true),  -- [CONFIRM] scrap %
  ('00000000-0000-4000-8000-000000000001', '1.4301', '1.4301 (AISI 304) stainless', 'stainless', 7900, 600,
    '[{"maxThicknessMm":999,"pricePerKg":3.60}]'::jsonb,  -- [CONFIRM] €/kg flat
    '[{"lengthMm":3000,"widthMm":1500},{"lengthMm":2500,"widthMm":1250},{"lengthMm":2000,"widthMm":1000}]'::jsonb,
    25.00, true),  -- [CONFIRM] scrap %
  ('00000000-0000-4000-8000-000000000001', 'AlMg3', 'AlMg3 EN AW-5754 aluminium', 'aluminium', 2660, 220,
    '[{"maxThicknessMm":999,"pricePerKg":4.50}]'::jsonb,  -- [CONFIRM] €/kg flat
    '[{"lengthMm":3000,"widthMm":1500},{"lengthMm":2500,"widthMm":1250},{"lengthMm":2000,"widthMm":1000}]'::jsonb,
    25.00, true),  -- [CONFIRM] scrap %
  ('00000000-0000-4000-8000-000000000001', 'CuZn37', 'CuZn37 (CW508L) brass', 'brass', 8440, 350,
    '[{"maxThicknessMm":999,"pricePerKg":8.50}]'::jsonb,  -- [CONFIRM] €/kg flat — no benchmark yet
    '[{"lengthMm":3000,"widthMm":1500},{"lengthMm":2500,"widthMm":1250},{"lengthMm":2000,"widthMm":1000}]'::jsonb,
    25.00, true),  -- [CONFIRM] scrap %
  ('00000000-0000-4000-8000-000000000001', 'Cu-ETP', 'Cu-ETP (CW004A) copper', 'copper', 8940, 250,
...
-- Mode "time": cost = (cutLengthM / speed + pierces × pierce_s / 60) / 60 ×
-- machine_rate + pierces × price_per_pierce. The base table is the 12 kW
-- mild-steel placeholder from Step 9 ("replace with TRUMPF cutting data");
-- it applies to all three mild-steel grades. Stainless, aluminium, brass and
-- copper run at 70 % of those speeds with the same pierce times, nitrogen,
-- up to the TruFiber 12001 thickness limit per family (12.7 / 6 / 6 / 6 mm).
-- Gas: mild steel O2 from 3 mm, N2 below; the other families N2.
-- min_contour_mm = 10 × t: contours below it get the slow-contour factor.
with base (thickness_mm, speed_m_min, pierce_s) as (
  values
    ( 1.0, 25.0, 0.2),  -- [CONFIRM]  1 mm: 25   m/min, 0.2 s pierce
    ( 2.0, 16.0, 0.3),  -- [CONFIRM]  2 mm: 16   m/min, 0.3 s pierce
    ( 3.0, 11.0, 0.4),  -- [CONFIRM]  3 mm: 11   m/min, 0.4 s pierce
    ( 4.0,  7.0, 0.6),  -- [CONFIRM]  4 mm:  7   m/min, 0.6 s pierce
    ( 5.0,  5.5, 0.8),  -- [CONFIRM]  5 mm:  5.5 m/min, 0.8 s pierce
    ( 6.0,  4.5, 1.0),  -- [CONFIRM]  6 mm:  4.5 m/min, 1.0 s pierce
    ( 8.0,  3.0, 1.3),  -- [CONFIRM]  8 mm:  3.0 m/min, 1.3 s pierce
    (10.0,  2.2, 1.6),  -- [CONFIRM] 10 mm:  2.2 m/min, 1.6 s pierce
    (12.0,  1.7, 2.0)   -- [CONFIRM] 12 mm:  1.7 m/min, 2.0 s pierce
)
...
-- 4a. Mild steel S235 / S355 / DC01 — in-house, time mode, 1–12 mm.
select
...
union all
-- 4b. Stainless 1.4301 (≤ 12 mm), aluminium AlMg3, brass CuZn37, copper
--     Cu-ETP (≤ 6 mm) — in-house, time mode, 70 % of the mild-steel speed.
select
  '00000000-0000-4000-8000-000000000001'::uuid,
  m.code,
  b.thickness_mm,
  ...
  true
from base b
cross join (values ('1.4301', 12.0), ('AlMg3', 6.0), ('CuZn37', 6.0), ('Cu-ETP', 6.0)) as m (code, max_thickness_mm)
where b.thickness_mm <= m.max_thickness_mm
on conflict do nothing;

-- 4c. Subcontracted cutting above the TruFiber limit (12.7 mm): S235 / S355
--     at 15 and 20 mm, priced per metre from the supplier tariff (spec 8.1,
--     13.3). The pricing engine switches to these rows and flags the part.
insert into public.rate_laser (
  rate_version_id, material_code, thickness_mm, mode, speed_m_min, pierce_s,
  price_per_m, price_per_pierce, gas, min_contour_mm, in_house, supplier, placeholder
) values
  ('00000000-0000-4000-8000-000000000001', 'S235', 15.0, 'per_m', null, null, 6.00, 0.50, null, null, false, '[CONFIRM] subcontractor', true),  -- [CONFIRM] 6.00 €/m, 0.50 €/pierce
  ('00000000-0000-4000-8000-000000000001', 'S235', 20.0, 'per_m', null, null, 8.00, 0.50, null, null, false, '[CONFIRM] subcontractor', true),  -- [CONFIRM] 8.00 €/m, 0.50 €/pierce
  ('00000000-0000-4000-8000-000000000001', 'S355', 15.0, 'per_m', null, null, 6.00, 0.50, null, null, false, '[CONFIRM] subcontractor', true),  -- [CONFIRM] 6.00 €/m, 0.50 €/pierce
  ('00000000-0000-4000-8000-000000000001', 'S355', 20.0, 'per_m', null, null, 8.00, 0.50, null, null, false, '[CONFIRM] subcontractor', true)   -- [CONFIRM] 8.00 €/m, 0.50 €/pierce
on conflict do nothing;

-- ─── 5. Tube-laser rates (spec 7 "tube_laser_rates", 8.2) ──────────────────
-- profile family × wall thickness → €/m of cut + handling per part + setup.
-- Placeholder ladder 4–10 €/m by wall; "open" profiles are not seeded.
...
-- thickness × length class (bend length ≤ class) → € per bend; +50 % above
-- 6 mm; setup € per part type spread over the quantity.
...
-- thickness 1–6 mm (machine limit) × radius class (radius ≤ class) → €/m of
-- roll axis + setup. One flat placeholder until the admin differentiates.
...
insert into public.rate_thread (rate_version_id, size, price_each, placeholder) values
  ('00000000-0000-4000-8000-000000000001', 'M3',      0.60, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M4',      0.65, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M5',      0.70, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M6',      0.80, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M8',      0.90, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M10',     1.00, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M10x1',   1.05, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M12',     1.20, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M12x1.5', 1.25, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M16',     1.40, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'M20',     1.50, true)   -- [CONFIRM]
on conflict do nothing;
...
insert into public.rate_feature (rate_version_id, code, name, price_each, placeholder) values
  ('00000000-0000-4000-8000-000000000001', 'countersink', 'Countersink',     0.80, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'counterbore', 'Counterbore',     1.20, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'bore_h7',     'H7 bore',         4.00, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'insert',      'Threaded insert', 1.50, true),  -- [CONFIRM]
  ('00000000-0000-4000-8000-000000000001', 'stud',        'Weld stud',       1.00, true)   -- [CONFIRM]
on conflict do nothing;
...
insert into public.rate_finish (rate_version_id, code, name, unit, price, minimum, placeholder) values
  ('00000000-0000-4000-8000-000000000001', 'powder',  'Powder coating',      'm2', 14.00, 25.00, true),  -- [CONFIRM] 14 €/m², min 25 €
  ('00000000-0000-4000-8000-000000000001', 'zinc',    'Zinc plating',        'kg',  1.20, 30.00, true),  -- [CONFIRM] 1.20 €/kg, min 30 € (supplier tariff)
  ('00000000-0000-4000-8000-000000000001', 'deburr',  'Deburring',           'm',   0.40,  0.00, true),  -- [CONFIRM] 0.40 €/m of cut
  ('00000000-0000-4000-8000-000000000001', 'engrave', 'Engraving / marking', 'm',   0.50,  0.00, true)   -- [CONFIRM] 0.50 €/m of tagged line
on conflict do nothing;

-- ─── 12. Machine park (spec 8, build prompt Step 9 — confirmed 25 Sep 2026) ─
-- limits JSON = lib/pricing/types.ts FlatLaserLimits / TubeLaserLimits /
-- PressBrakeLimits / RollLimits / WeldLimits. Edit in the admin machines
-- editor; a re-run of this file does not overwrite existing rows.
